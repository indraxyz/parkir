@extends('admin.layout') @section('title') Kelola Blok @endsection
@section('content')
<h2 class="text-2xl mb-3">Kelola Blok</h2>

<div class="flex items-center justify-between mb-2 mt-5">
    <form
        action="{{ url('admin/master/blok-cari') }}"
        method="get"
        class="w-full "
    >
        @csrf
    ...
    <a href="{{ url('admin/master/blok-add') }}" class="">
        <svg
            viewBox="0 0 20 20"
            fill="currentColor"
...
<table class="table-auto w-full mb-5">
    ...
    <tbody>
        ...
        <tr data-id="x">
            ...
            <td class="border px-4 py-2">{{$blok->nama}}</td>
            <td class="border px-4 py-2">
                {{$blok->keterangan}}
            </td>
            <td class="border px-4 py-2 w-20">
                <div class="flex items-center">
                    <a href="{{ url('admin/master/blok-edit/'.$blok->id) }}">
                        <svg
                            viewBox="0 0 20 20"
                            fill="currentColor"
                    ...
                    <a href="{{ url('admin/master/blok-hapus/'.$blok->id) }}">
                        <svg
                            viewBox="0 0 20 20"
                            fill="currentColor"
